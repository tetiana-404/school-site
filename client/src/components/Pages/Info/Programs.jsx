...
const ProgramsEditor = ({ user }) => {
  ...
  const fetchProgramsDocuments = async () => {
    try {
      const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/programs`);
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint = editing
      ? `${process.env.REACT_APP_BACKEND_URL}/programs/${editing.id}`
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Видалити документ?')) {
      try {
        await fetch(`${process.env.REACT_APP_BACKEND_URL}/programs/${id}`, {
          method: 'DELETE',
        });
        await fetchProgramsDocuments();
      } catch (error) {
