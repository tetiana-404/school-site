...
const ProgramsEditor = ({ user }) => {
  ...
  const fetchProgramsDocuments = async () => {
    try {
      const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/programs`);
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint = editing
      ? `${process.env.REACT_APP_BACKEND_URL}/api/programs/${editing.id}`
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Видалити документ?')) {
      try {
        await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/programs/${id}`, {
          method: 'DELETE',
        });
        await fetchProgramsDocuments();
      } catch (error) {
  ...
  return (
    <section id="programsPage" className="section-padding bg-light py-5">
      <div className="auto-container">
        ...
        <div className="row">
          <div className="col-lg-12">
            <div className="py-3">
              <img src={process.env.PUBLIC_URL + '/img/Освітні-програми.jpg'} className='w-100' />
            </div>
